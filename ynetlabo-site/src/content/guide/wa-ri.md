...
app: "wa/ri"
updated: 2026-08-26T00:31:24.537Z
draft: false
...
| 3 | まとめて1件にする | 通常の記録に戻す。明細をすでに入れていると「入力した明細を破棄しますか？」の確認が入る。 |
| 4 | 会計メモ | この会計に付ける名前。一覧では会計の小見出しになる。空のままなら「まとめて記録」と表示される。 |
| 5 | 明細の行 | カテゴリ・品名・割り勘の対象・金額。**タップで編集**。 |
| 6 | 明細を追加 | 明細を1件足す。 |
| 7 | この会計での各自の負担 | 明細を入れるたびに更新される。レシートと突き合わせてから登録できる。 |
| 8 | 「N件を記録する」 | 明細の件数ぶんの記録を1回でまとめて登録する。 |
...
| 5 | カテゴリ | 明細ごとに変えられる。新規登録のときだけ先頭に「会計と同じ」が出る。 |
...
| No. | 場所 | 説明 |
|---|---|---|
| 1 | 会計の小見出し | 会計メモ・小計・件数。タップでその会計の明細を畳む／開く。画面を開き直すと必ず開いた状態に戻る。 |
