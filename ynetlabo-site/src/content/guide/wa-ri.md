...
app: "wa/ri"
updated: 2026-08-08T02:31:53.578Z
draft: false
...
はじめて開くと、全5ページのチュートリアルが出る。あとから 設定 → チュートリアルでいつでも見直せる。
...
- サーバに繋がらないときは、トップメニューではなく[オフライン画面](#20-圏外オフラインのとき)に入る。
...
作り方と引き継ぎは [19. アカウントの管理・引き継ぎ・退会](#19-アカウントの管理引き継ぎ退会)へ。
...
- 通貨だけは後から変えられない。海外旅行で使うなら作るときに決める（→ [16. 海外旅行で使う](#16-海外旅行で使う外貨ルーム)）。
...
| No. | 場所 | 説明 |
|---|---|---|
| 9 | メンバーの設定 | 割り勘の対象になる人を登録する。オーナー以外は閲覧のみ。 |
| 10 | ルームの設定 | ルーム名・アイコン・確定・メンバー編集許可。オーナー限定。 |
| 11 | CSV出力 | 記録をCSVに書き出す。実行前に広告の視聴確認が出る。 |
| 12 | PDF出力 | 同じ内容をPDFで書き出す。記録が0件のときは「出力できる記録がありません」で止まる。 |
| 13 | ルームから抜ける | 招待された人だけに出る。記録は消えない。 |
| 14 | ルームを削除する | オーナーのみ。記録ごと消える。**元に戻せない**。 |
...
|---|---|---|
| 1 | ヘルプ | この画面の使い方ガイドを開く。 |
| 2 | 支払／集金 | 切り替え。集金については [15. 集金を記録して精算を追う](#15-集金を記録して精算を追う)へ。**既存の記録を編集するときは切り替えられない**。 |
...
| 6 | 割り勘の対象 | 初期状態は全員が選択済み。タップで外す／戻す。 |
| 7 | 一時記録から復元 | 控えておいたメモから日付・金額・カテゴリ・メモを流し込む（→ [13. 一時記録](#13-あとで登録する一時記録)）。 |
| 8 | カメラで読み取る | レシートの合計金額を自動で読み取る（→ [14. レシートをカメラで読み取る](#14-レシートをカメラで読み取る)）。 |
| 9 | 金額 | 1以上の整数。外貨ルームでは通貨バッジが出る。 |
| 10 | カテゴリ | 飲食・交通・宿泊・アクティビティ・その他など。集金モードでは集金用だけが並ぶ。 |
| 11 | 連続登録 | ONにすると登録後もシートが閉じず、メンバー選択を保ったまま次の記録を入力できる。 |
| 12 | 「登録」ボタン | 確定済みルームでは「このルームの記録は確定済みです。」で拒否される。 |
...
| 4 | CSV出力 | 記録をCSVに書き出す。実行前に広告の視聴確認が出る。 |
| 5 | PDF出力 | 同じ内容をPDFで書き出す。 |
...
- 確定したルームを一覧から隠したいときは、[18. 設定でできること](#18-設定でできること)の表示設定で「確定済みを隠す」をONにする。
